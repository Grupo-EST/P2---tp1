...
public class EditorConsulta extends JDialog {
	...
	private EditorConsulta( Point posicao, GEstSaude g, Consulta consulta, String titulo ) {
		...
		if( consulta != null ) {
			...
			snsUtente.setText( consulta.getUtente().getNumSns() ); 
			testaIdUtente();
			
			// TODO colocar aqui o identificador do serviço 
			idServico.setSelectedItem( consulta.getServico().getId() );
			testaIdServico();
	...
	protected void testaIdServico() {
		...
		if( servico == null ) {
			...
			nomeServico.setText( servico.getDescricao() );
			testaTudoOk();
		}
	}
	...
	protected void okPremido() {
		...
		consultaRes = new Consulta(quando, servico, utente);
		setVisible( false );
	}
